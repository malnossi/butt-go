...
DEPS_DIST="${WORKSPACE_DIR}/deps-dist-windows"
LAME_VERSION="3.100"
...
echo "--- Building Static Dependencies for Windows ---"
mkdir -p "${DEPS_DIST}"

export CC=${HOST}-gcc
export CXX=${HOST}-g++
# We might need some flags to suppress warnings/errors or properly link
# LAME might need AR and RANLIB set as well
export AR=${HOST}-ar
...
./configure --host=${HOST} --prefix="${DEPS_DIST}" --enable-static --disable-shared --disable-frontend
...
cd portaudio
./configure --host=${HOST} --prefix="${DEPS_DIST}" --enable-static --disable-shared --with-winapi=wmme,directx,wasapi
make -j$(nproc 2>/dev/null || echo 4)
make install
...
cd fdk-aac-2.0.2
./configure --host=${HOST} --prefix="${DEPS_DIST}" --enable-static --disable-shared
make -j$(nproc 2>/dev/null || echo 4)
make install
...
cd libvorbis-1.3.7
./configure --host=${HOST} --prefix="${DEPS_DIST}" --enable-static --disable-shared --with-ogg="${DEPS_DIST}"
make -j$(nproc 2>/dev/null || echo 4)
make install
...
PKG_CONFIG_PATH="${DEPS_DIST}/lib/pkgconfig" CFLAGS="-I${DEPS_DIST}/include" LDFLAGS="-L${DEPS_DIST}/lib" ./configure --host=${HOST} --prefix="${DEPS_DIST}" --enable-static --disable-shared --disable-http
make -j$(nproc 2>/dev/null || echo 4)
make install

echo "Done building Windows static dependencies!"
